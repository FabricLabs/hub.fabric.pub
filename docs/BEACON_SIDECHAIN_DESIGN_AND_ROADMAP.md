...
8. **Multi-sign UX** — Collect threshold sigs for patches/epochs via delegation / desktop signing flows already used for execution contracts. **Epoch rounds:** `FederationSignRequest` broadcast + RPC `SubmitBeaconEpochSignature` / `ListPendingBeaconEpochSignatures` (ADR-001). Core `Beacon#submitFederationEpochSignature` **idempotently finalizes** already-`ready` rounds if persist failed (does not reopen for new sigs).
...
| File | Responsibility |
|------|------------------|
| `contracts/beacon.js` | Thin Hub subclass of `@fabric/core` `types/beacon` (Hub name default; epoch chain, reorg prune, fail-closed witnesses, and ready-round retry finalize live in core) |
