...
| **J** | **Peer inventory accumulate + markup republish** (opt-in): persist remote `INVENTORY_RESPONSE` rows; republish **held** files at `markupBps` + `markupSats`; offer book on Documents / `ListDocumentOffers`. Does not advertise files this hub cannot fulfill. | Shipped |
