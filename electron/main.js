...
if (!electron.ipcMain) {
  console.error('[DESKTOP] Run the Electron main process with the Electron binary, e.g. ./node_modules/.bin/electron electron/main.js (not `node electron/main.js` or a mis-resolved `npx electron`).');
  process.exit(1);
}
...
const { runDesktopHubStartupProbe } = require('./desktopHubProbe');
    ...
    function ping () {
      const req = http.get(
        ...
        (res) => {
            ...
            const looksLikeHub =
              body &&
              (body.includes('application-target') || body.includes('browser.min.js'));
            if (statusOk && looksLikeHub) {
              resolve();
              return;
            }
            if (Date.now() - start > timeoutMs) {
              reject(
                new Error(
                  `Hub HTTP on ${hubUrl()} not ready (status ${res.statusCode || 0}, hub shell markers missing). ` +
                    'Check that port ' +
                    HUB_PORT +
                    ' is the Fabric Hub (not another app) and that `npm run build:browser` has been run.'
                )
              );
              return;
            }
 ...
 * In dev, `electron electron/main.js` makes `app.getAppPath()` point at `electron/`, not the repo root — use `__dirname`.
 */
function getAppRoot () {
  if (app.isPackaged) {
