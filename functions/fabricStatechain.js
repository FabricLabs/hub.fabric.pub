...
/**
 * Vendored sidechain document helpers (fallback when `@fabric/core/functions/sidechainState`
 * is not linked). Prefer the core module. **Not a Fabric type.**
 *
 * A sidechain document (Hub: “sidechain state”) is a logical JSON value
 * `{ version, clock, content }` advanced by RFC6902 patches, authorized by federation
 * (or operator policy), and sealed into L1-tied epoch digests with full snapshots for
 * reorg rewind. Optional path policy encodes the contract’s allowed mutations.
 *
...
function patchesForNamespaceHead (existingContent, contractId, head) {
  ...
  if (!namespaces) {
    return [{
      op: 'add',
      path: `/${key}`,
      value: { [id]: DistributedExecution.jsonSafe(head) }
    }];
  }
  if (prev) {
    return [{ op: 'replace', path, value: DistributedExecution.jsonSafe(head) }];
  }
  return [{ op: 'add', path, value: DistributedExecution.jsonSafe(head) }];
}
...
function appendJournalEntrySync (fs, entry) {
  ...
  const row = {
    ...
    patches: DistributedExecution.jsonSafe(entry.patches || []),
...
function encodeSidechainStatePatchMessage (proposal) {
  return {
    ...
    patches: DistributedExecution.jsonSafe(proposal.patches || []),
...
function verifyEpochChainFederationWitnesses (epochChain, validators, threshold) {
  ...
  for (const e of chain) {
    ...
    const ok = DistributedExecution.verifyFederationWitnessOnMessage(buf, e.federationWitness, pubs, thr);
    if (!ok) {
      failures.push({
