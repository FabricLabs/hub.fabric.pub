...
- ~~**`@fabric/core` / `@fabric/http` pin hygiene**~~ — pins: core `2e4d2ac75e8e…`, http `9c9e8f823d11…` (refreshed via `feature/rsi`; plain `report:install` keeps the lockfile — bump with `npm install …#feature/rsi --allow-git=all`). Keep `package.json` on moving `feature/rsi` during RSI; re-pin releases to lockfile SHAs.
