...
- ~~**`@fabric/core` / `@fabric/http` pin hygiene**~~ — pins: core `9938917804e2bf5ba5cf1fab7bf0975129d9063f`, http `7d7f1c7c918dabe7f2ae59d638b16e1a020d08bd` (refreshed via `feature/rsi`; `report:install` removes `package-lock.json` then `npm i --allow-git=all`). Keep `package.json` on moving `feature/rsi` during RSI; re-pin releases to lockfile SHAs.
