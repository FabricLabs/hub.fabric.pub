...
**HTML-only / CDN:** serving `assets/` without `scripts/hub.js` is a supported UI mode. The SPA classifies `GET /settings` (JSON vs HTML/CDN protection) in `functions/hubClientEnvironment.js` and shows only client-exclusive features until Hub HTTP is present. Do not hide operator chrome when that probe is Hub JSON. Point a CDN build at live Hubs with **`window.FABRIC_HUB_SEEDS`** (or env **`FABRIC_HUB_SEEDS`**) so the client can `OPTIONS /` those origins, join their WebRTC mesh, probe `GET /services/peering`, and list Documents from peer inventories. **`/downloads`** is always available: it browses `assets/downloads/index.json` (generated at build) and serves installer files as static assets. **Site login (Passport / desktop `fabric://login`) is not a CDN goal** — it needs same-origin `/sessions` on a live Hub or the goon.vc zipper proxy; HTML-only Identity offers local key create/restore only.
