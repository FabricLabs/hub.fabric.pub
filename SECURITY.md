...
- ~~**`@fabric/core` / `@fabric/http` pin hygiene**~~ — pins: core `1c3f8d08cbad10cc71538886723b88fca0db03e2`, http `bbdb72a580e3286ab5984234058115892cb4bdd4` (refreshed via `feature/rsi`; `report:install` removes `package-lock.json` then `npm i --allow-git=all`). **Push http `feature/rsi` through `bbdb72a` before Hub CI can resolve the lockfile.** Keep `package.json` on moving `feature/rsi` during RSI; re-pin releases to lockfile SHAs.
