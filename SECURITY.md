...
- ~~**`@fabric/core` / `@fabric/http` pin hygiene**~~ — pins: core `0ed61d62057e1bee719a19941201eeecd66ca864`, http `61bb801aa0c6a79b284af1f29466183ef9e6b7d4` (refreshed via `feature/rsi`; `report:install` wipes the lockfile then `npm i --allow-git=all`). Keep `package.json` on moving `feature/rsi` during RSI; re-pin releases to lockfile SHAs.
