...
- ~~**`@fabric/core` / `@fabric/http` pin hygiene**~~ — pins: core `ab0acf77b…`, http `270ebbb…` (refreshed via `feature/rsi`; `report:install` wipes the lockfile then `npm i --allow-git=all`). Keep `package.json` on moving `feature/rsi` during RSI; re-pin releases to lockfile SHAs.
