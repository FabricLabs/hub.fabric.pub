...
- ~~**`@fabric/core` / `@fabric/http` pin hygiene**~~ — pins: core `488a87da1…`, http `5161e76…` (refreshed via `feature/rsi`; `report:install` wipes the lockfile then `npm i --allow-git=all`). Keep `package.json` on moving `feature/rsi` during RSI; re-pin releases to lockfile SHAs.
