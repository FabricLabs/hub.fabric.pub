...
| **L1 verification** | `_l1PaymentVerificationDetail` (`getrawtransaction`, or wallet `gettransaction` + `decoderawtransaction` for confirmed wallet sends), `GET .../transactions/:txid?address=&amountSats=` / RPC / WS with `confirmations`, `inMempool`, `matchedSats`. |
...
- **E2E:** Two-hub inventory HTLC L1 mocha **`npm run test:e2e-inventory-htlc`** (`tests/hub.inventory.htlc.e2e.test.js`) **passes** (opt-in; needs `bitcoind`). Puppeteer **screenshot walkthrough** of Documents / PeerView HTLC: **`npm run screenshots`** (UI) / **`npm run screenshots:l1`** (L1) → [USER_FLOWS.md](USER_FLOWS.md) (`data-testid="peer-inventory-htlc"`). Phase E human acceptance still tracks in [PRODUCTION_ROADMAP.md](PRODUCTION_ROADMAP.md). Same-hub invoice claim stays `npm run test:e2e-document-purchase`.
