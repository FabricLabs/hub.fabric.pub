...
1. **Client-signed player login** (Passport, GoonCitizen, any Fabric wallet) — body `{ signature, pubkeyHex, identity: { id, xpub } }` where `signature` is BIP340 Schnorr over the **server-stored** challenge. Crypto verification is the authenticator; the Hub does **not** use its root key.
