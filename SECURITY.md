...
- ~~**`@fabric/core` / `@fabric/http` pin hygiene**~~ — pins: core `51ad619c…`, http `f81feb31…` (refreshed via `feature/rsi`; plain `report:install` keeps the lockfile — bump with `npm install …#feature/rsi --allow-git=all`). Keep `package.json` on moving `feature/rsi` during RSI; re-pin releases to lockfile SHAs.
...
- **Site-login / device-link Origin gates** — inherited from `@fabric/http` (forgeable Origin/Referer for session/device-link redeem on shared hosts; Hub self-sign is now opt-in + loopback-only in http). Prefer possession proofs before production shared bind; cleartext production hubs are no longer default-allowlisted.
...
- ~~**Beacon federation round close**~~ — core tip `addSignature` rejects further sigs when status is `ready` or `sealed`; Hub `createRound` wrapper defaults omitted `policy` to `{}`.
