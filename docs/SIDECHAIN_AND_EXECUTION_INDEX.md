...
- **PR #15 RSI:** remaining heavy lifts (encrypted xprv import persistence, stronger local at-rest encryption, PR split) live in [SECURITY.md](../SECURITY.md) Outstanding. Already landed / staged hardening includes: document-action auth + edit failure checks, crowdfund BIP44 account alignment, Hub admin-token scoping for non-Hub bitcoin bases, inventory HTLC `preimage`/reveal fan-out, desktop `waitForHub` request timeout, WebRTC chat relay envelope, forget/destroy session key wipe, inventory offer-reply merge aliases, identity capability `masterXprv` detection, encrypted-backup gating, immutable Fabric dep SHAs (`0e1fa192…` / `e8f70a32…`), timing-safe xpub query tokens, compressed-pubkey Bech32 id validation, core-aligned Fabric coin-type paths (default 7778; optional mainnet 7777), `@fabric/http` 402 resolve via package exports, masterXpub UI labels, payment test-route opt-in, wallet-summary cache ages, and `prepare-hub-dev-port` `execFileSync` hardening.
