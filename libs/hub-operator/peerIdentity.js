...
function fabricPeerRecencyMs (peer) {
  ...
  for (const value of values) {
    const n = coercePeerTimestampMs(value);
    if (n > best) best = n;
  }
  return best;
}
...
function findFabricPeerRow (peers, actorId) {
  ...
  if (identityHits.length) {
    return identityHits.find((p) => p && p.status === 'connected') || identityHits.find(() => true) || null;
  }
  ...
  return addrHits.find((p) => p && p.status === 'connected') || addrHits.find(() => true) || null;
}
...
function consolidateUnifiedPeersByFabricId (peers) {
  ...
  for (const [, rows] of groups) {
    ...
    for (const row of rows) {
      merged = merged == null ? row : mergeFabricPeerRows(merged, row);
    }
