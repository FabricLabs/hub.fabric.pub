...
**HTML-only / CDN:** serving `assets/` without `scripts/hub.js` is a supported UI mode. The SPA classifies `GET /settings` (JSON vs HTML/CDN protection) in `functions/hubClientEnvironment.js` and shows only client-exclusive features until Hub HTTP is present. Do not hide operator chrome when that probe is Hub JSON. **Public visitors** (no enrolled Fabric identity) use `functions/hubPublicVisitor.js`: TopPanel / Settings / Features strip operator cards; operator routes stay behind `PublicVisitorGate`. Point a CDN build at live Hubs with **`window.FABRIC_HUB_SEEDS`** (or env **`FABRIC_HUB_SEEDS`**) so the client can `OPTIONS /` those origins, join their WebRTC mesh, probe `GET /services/peering`, and list Documents from peer inventories. **`/downloads`** is always available: it browses `assets/downloads/index.json` (generated at build) and serves installer files as static assets. **Site login (Passport / desktop `fabric://login`) is not a CDN goal** — it needs same-origin `/sessions` on a live Hub or the goon.vc zipper proxy; HTML-only Identity offers local key create/restore only.
...
| `PublishFabricHallmark` | Params `{ adminToken, tipBlockHash? }` — **regtest + named wallet only**: OP_RETURN 40-byte Fabric hallmark (tip + Hub contract + latest sidechain/contracts digests); on-chain only |
...
Prefer the Fabric Environment from **`fabric setup`**: password-sealed `~/.fabric/wallet.json` (`FABRIC_PASSWORD`, idle lock via `FABRIC_LOCK_TIMEOUT_MINUTES`). Process env **`FABRIC_XPRV`** (xprv, or watch-only xpub/pubkey) then **`FABRIC_SEED`** / **`FABRIC_MNEMONIC`** override the wallet. Hub collapses mixed key bags with `@fabric/core/functions/fabricOperatorIdentity` (xprv-first). Optional local automation: **`local/fabric-operator-identity.json`** (gitignored) is a **fallback only** — `npm run operator-identity:init`, `operator-identity:emit-hub-config`, `operator-identity:sync`, `operator-identity:print-env`. Never commit identity files or `assets/config.local.js` when they contain secrets.
...
**`npm run desktop`** runs a preflight (`scripts/ensure-fabric-linked.js`) so missing deep modules (**`@fabric/http/types/distributedExecutionHttp`**, **`@fabric/core/types/machine`**, **`@fabric/core/functions/beaconFederationSigning`**, **`@fabric/core/types/chain`**, **`@fabric/core/functions/sidechainState`**) fail fast with a **`npm run link:fabric`** hint.
