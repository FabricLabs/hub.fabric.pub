...
- ~~**`@fabric/core` / `@fabric/http` pin hygiene**~~ — pins: core `5557a2bfc830a3b472e6d26ed46b1a5fd7c16b9b`, http `7536341d8fb910c9668c3f95e8cadfd5381523de` (refreshed via `feature/rsi`; `report:install` wipes the lockfile then `npm i --allow-git=all`). Keep `package.json` on moving `feature/rsi` during RSI; re-pin releases to lockfile SHAs.
